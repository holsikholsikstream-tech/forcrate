...
public class WinHandler {
    ...
    private boolean isGamepadInputSuppressedForClient(boolean requestedXInput) {
        return xinputDisabled && useLegacyInputMethod && requestedXInput;
    }

    private byte getReportedInputMode(boolean requestedXInput) {
        if (useLegacyInputMethod) {
            if (xinputDisabled && dinputMapperType == DINPUT_MAPPER_TYPE_XINPUT) {
                return DINPUT_MAPPER_TYPE_STANDARD;
            }
            return dinputMapperType;
        }

        if (xinputDisabled) {
            if (requestedXInput || inputType == FLAG_INPUT_TYPE_XINPUT || inputType == DEFAULT_INPUT_TYPE) {
                return FLAG_INPUT_TYPE_DINPUT;
            }
        }
    ...
    private ExternalController bindController(ExternalController controller) {
        if (controller == null) {
            return null;
        }
    ...
    private ExternalController resolveController(Integer preferredDeviceId) {
        ExternalController controller = currentController;

        if (controller != null && controller.isConnected()) {
            if (preferredDeviceId == null || preferredDeviceId == 0 || controller.getDeviceId() == preferredDeviceId) {
                return controller;
            }

            ExternalController byCurrentId = ExternalController.getController(controller.getId());
            if (byCurrentId != null && (preferredDeviceId == null || preferredDeviceId == 0 || byCurrentId.getDeviceId() == preferredDeviceId)) {
                return bindController(byCurrentId);
            }
        }

        if (preferredDeviceId != null && preferredDeviceId != 0) {
            ExternalController preferred = ExternalController.getController(preferredDeviceId);
            if (preferred != null) {
                return bindController(preferred);
            }
        }

        if (controller != null) {
            ExternalController byDescriptor = ExternalController.getController(controller.getId());
            if (byDescriptor != null) {
                return bindController(byDescriptor);
            }
        }

        return bindController(ExternalController.getController(0));
    }

    private GamepadState buildOutputState(GamepadState sourceState) {
        GamepadState outputState = new GamepadState();
        outputState.copy(sourceState);
        outputState.thumbRX = Mathf.clamp(outputState.thumbRX + gyroX, -1.0f, 1.0f);
        outputState.thumbRY = Mathf.clamp(outputState.thumbRY + gyroY, -1.0f, 1.0f);
        return outputState;
    }
    ...
    private void handleRequest(byte requestCode, final int port) {
        switch (requestCode) {
            ...
            case RequestCodes.GET_GAMEPAD: {
                boolean requestedXInput = receiveData.get() == 1;
                boolean notify = receiveData.get() == 1;
                final ControlsProfile profile = activity.getInputControlsView().getProfile();
                final boolean useVirtualGamepad = profile != null && profile.isVirtualGamepad();
                final ExternalController controller = !useVirtualGamepad ? resolveController(null) : null;
                final boolean enabled = !isGamepadInputSuppressedForClient(requestedXInput) && (controller != null || useVirtualGamepad);
                final byte reportedInputMode = getReportedInputMode(requestedXInput);
                ...
                addAction(() -> {
                    ...
                    if (enabled) {
                        sendData.putInt(!useVirtualGamepad ? controller.getDeviceId() : profile.id);
                        sendData.put(reportedInputMode);

                        byte[] bytes = (useVirtualGamepad ? profile.getName() : controller.getName()).getBytes();
                        sendData.putInt(bytes.length);
                        sendData.put(bytes);
                    } else {
                        sendData.putInt(0);
            ...
            case RequestCodes.GET_GAMEPAD_STATE: {
                ...
                final ControlsProfile profile = activity.getInputControlsView().getProfile();
                final boolean useVirtualGamepad = profile != null && profile.isVirtualGamepad();
                final ExternalController controller = !useVirtualGamepad ? resolveController(gamepadId) : null;
                ...
                addAction(() -> {
                    ...
                    if (enabled) {
                        sendData.putInt(!useVirtualGamepad ? controller.getDeviceId() : gamepadId);
                        GamepadState state = useVirtualGamepad ? buildOutputState(profile.getGamepadState()) : buildOutputState(controller.state);
    ...
    public void setXInputDisabled(boolean disabled) {
        ...
        Log.d("WinHandler", "XInput Disabled set to: " + xinputDisabled + ", inputType=" + inputType + ", legacy=" + useLegacyInputMethod);
    }
...
//    }
    ...
    public boolean onKeyEvent(KeyEvent event) {
        boolean handled = false;
        ExternalController controller = ExternalController.isGameController(event.getDevice()) ? resolveController(event.getDeviceId()) : currentController;
        ...
        if (event.getKeyCode() == gyroTriggerButton) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                ...
                if (controller != null) {
                    controller.state.thumbRX = 0.0f; // Reset X axis
                    controller.state.thumbRY = 0.0f; // Reset Y axis
                }
        ...
        if (controller != null && controller.getDeviceId() == event.getDeviceId() && event.getRepeatCount() == 0) {
    ...
    public void initializeController() {
        ExternalController controller = resolveController(null);
        if (controller != null) {
            Log.d("WinHandler", "Controller initialized: " + controller.getName() + " (" + controller.getId() + ")");
        }
    }
